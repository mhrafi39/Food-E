...
import { Plus, Edit, Trash2, Search, Package, ChefHat } from 'lucide-react';
              ...
              >
                ...
                <div className="p-6">
                  <div className="mb-2">
                    ...
                    {item.hasRecipe && (
                      <span className="ml-2 text-xs px-2 py-1 bg-green-500/20 text-green-300 rounded-full">
                        Has Recipe
                      </span>
                    )}
                  </div>
                    ...
                    {item.hasRecipe && (
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-semibold text-green-400">
                          {item.preparedStock || 0} ready to sell
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  {/* Recipe Ingredients */}
                  {item.hasRecipe && item.recipeIngredients && item.recipeIngredients.length > 0 && (
                    <div className="mb-4 p-3 bg-white/5 rounded-lg border border-white/10">
                      <div className="flex items-center gap-2 mb-2">
                        <ChefHat className="w-4 h-4 text-brand" />
                        <span className="text-xs font-semibold text-brand">Recipe ({item.recipeIngredients.length} ingredients)</span>
                      </div>
                      <div className="space-y-1">
                        {item.recipeIngredients.slice(0, 3).map((ing, idx) => (
                          <div key={idx} className="text-xs text-white/70 flex justify-between">
                            <span>{ing.rawMaterialName}</span>
                            <span className="font-semibold">{ing.quantityNeeded} {ing.unit}</span>
                          </div>
                        ))}
                        {item.recipeIngredients.length > 3 && (
                          <p className="text-xs text-white/50 italic">
                            +{item.recipeIngredients.length - 3} more...
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="mb-4 p-3 bg-white/5 rounded-lg border border-white/10">
                      <div className="text-xs text-white/70">
                        <div className="flex justify-between mb-1">
                          ...
                          <span className="font-semibold">৳{item.directPurchaseCost?.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Profit Margin:</span>
                          <span className="font-semibold text-green-400">৳{item.profitMargin?.toFixed(2)}</span>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex gap-2">
                    {item.hasRecipe && (
                      <Link
                        to={`/admin/items/${item.id}/recipe`}
                        className="flex-1 px-4 py-2 bg-green-500/20 text-green-300 rounded-lg font-semibold hover:bg-green-500/30 transition-colors flex items-center justify-center"
                      >
