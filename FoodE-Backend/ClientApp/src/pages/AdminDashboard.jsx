    ...
    const fetchDashboard = async () => {
      ...
      if (result.success) {
        ...
        const dashboardData = {
          ...
          todayProfit: result.data.todayProfit || 0,
          todayRevenue: result.data.todayRevenue || 0,
          todayCost: result.data.todayCost || 0,
          lowStockMaterials: result.data.lowStockMaterials || [],
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white/60">Check console for errors or ensure you're logged in as admin</p>
        </div>
      </div>
    );
  }
  ...
  const statsCards = [
    {
      ...
      value: `৳${(stats.todayProfit || 0).toFixed(2)}`,
      subtitle: `Cost: ৳${(stats.todayCost || 0).toFixed(2)} | Revenue: ৳${(stats.todayRevenue || 0).toFixed(2)}`,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          {stats.lowStockMaterials && stats.lowStockMaterials.length > 0 && (
            <div className="glass rounded-2xl p-6 border-2 border-yellow-500/50">
              ...
              <div className="space-y-3">
                {stats.lowStockMaterials.map((material) => (
                  <div key={material.id} className="flex items-center justify-between p-3 bg-yellow-500/10 rounded-lg">
                    <div>
                      <p className="font-semibold">{material.name}</p>
                      <p className="text-xs text-white/60">Minimum: {material.minimumStock} {material.unit}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-yellow-500">{material.currentStock} {material.unit}</p>
                      <Link to="/admin/materials" className="text-xs text-brand hover:underline">
                        Restock
                      </Link>
                    </div>
                  </div>
